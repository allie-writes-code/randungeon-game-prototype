...
/*function GenRoom()
{
	...
	// This var controls the size and amount of content in the room:
	var numOfLinesInRoom = getRandomInt(5,20);
	
	// Start a loop, using the above var to control:
	for (var i = 1; i <= numOfLinesInRoom; i++)
	{
		if (i === 1)
		{
			// If this is the first line, set the first line of roomString to an edge:
			roomString = roomString + roomEdge + newLine;
		}
		else if (i === numOfLinesInRoom)
		{
			// Else if this is the final room.
			// Check if the player has been spawned yet, if so we need to spawn them before finishing the room!
			
			// If they haven't:
			if (playerSpawned === false)
			{
				// Run the loop once more so we can finish the room.
				i--;
				
				// Add to roomString using a random element from the playerRoomm array:
				roomString = roomString + playerRoom[getRandomInt(0, playerRoom.length - 1)] + newLine;
				playerSpawned = true;
			}
			else
			{
				// If the player has spawned, finish the room:
				roomString = roomString + roomEdge + newLine;
			}			
		}
		else
		{
			// Else this must be one of the middle lines in the room, not an edge or a player spawn.
		
			var MLChance = getRandomInt(1,100)
			
			if (MLChance < 50)
			{
				if (playerSpawned === false)
				{
					var spawnPlayerChance = getRandomInt(0,3);
					...
					{
						roomString = roomString + roomEmpty[getRandomInt(0,roomEmpty.length - 1)] + newLine;
					}
				}
				else
				{
			...
			{
				var spawnQuestChance = getRandomInt(0, 3);
				
				if (questSpawned === false && spawnQuestChance === 3)
				{
					roomString = roomString + questRoom[getRandomInt(0, questRoom.length - 1)] + newLine;
					questSpawned = true;
				}
				else
				{
			...
			{
				roomString = roomString + mobRoom[getRandomInt(0, mobRoom.length - 1)] + newLine;
				numOfMobs++;
			}
			else
			{
				roomString = roomString + lootRoom[getRandomInt(0, lootRoom.length - 1)] + newLine;
				numOfLoot++;
			}
		}
	}
		...
		{	
			...
			for (var i = 0; i < currentNumOfMobs; i++)
			{
				if (currentMobs[i].name !== "DEAD MOB")
				{
					...
					var playerHit = getRandomInt(playerAtk, playerMaxHit);
